...
- Необязательная общая **Active queue** для всех шести действий агентов на одном компьютере; по умолчанию она выключена.
...
Включите **Active queue** в инспекторе свойств любого действия агента, чтобы собрать актуальные задачи на первых клавишах. Настройка действует сразу для всех действий от **Agent 1** до **Agent 6** на этом компьютере и по умолчанию выключена. Очередь получает задачи из полного встроенного каталога закреплённых и незакреплённых разговоров Codex: сначала идут задачи, требующие внимания, и ошибки, затем завершённые и непрочитанные задачи в прежнем порядке поступления при наличии времени активности, а после них — работающие задачи по времени последнего сообщения пользователя, которое начало работу. Открытие или выбор задачи, изменение заголовка, фоновые рассуждения и вызовы инструментов, ответ ассистента, активность renderer и обычное обновление снимка не меняют порядок работающих клавиш. Новое сообщение пользователя поднимает продолжающую работать задачу.

Для этого порядка Codex Deck читает только тип, временную метку и байтовое смещение структурного события `event_msg` с `payload.type === "user_message"`; текст сообщения не читается и не передаётся через ретранслятор. Если событие недоступно — например, его не отправляет старая версия, плагин запущен заново или запись осталась за пределами последних 512 КиБ файла сессии, — задача получает стабильное место внутри текущей очереди, а не вымышленное время старта. Задачи с известным стартом идут перед задачами без него. Это запасное место действует только в текущем цикле включения очереди: выключение и повторное включение **Active queue** или перезапуск плагина начинает новый цикл, а временно исчезнувшие записи сохраняются 24 часа. Задачи в состоянии простоя или `off` скрываются, остальные позиции сдвигаются без пропусков; на клавиши выводится не больше шести задач.

Если полный каталог временно недоступен или несовместим с текущей версией renderer, **Active queue** безопасно возвращается к существующим шести встроенным слотам Micro, не отключая обычный снимок состояния. Исправная чёрная позиция ничему не назначена и не реагирует на нажатие; сообщения о подключении, ухудшении связи и недоступности узла остаются видимыми.

Если в профиле N клавиш агентов, расположите логические действия **Agent 1** … **Agent N** подряд и по порядку. Пока очередь включена, открыть простаивающие разговоры этими клавишами нельзя. Закреплённые и незакреплённые задачи входят в полный встроенный каталог, а режим **custom** намеренно ограничен шестью назначенными кандидатами, среди которых очередь всё равно может уплотнить релевантные. Выключение **Active queue** полностью восстанавливает прежнюю раскладку источника агентов для одного или нескольких узлов.
