...
Включите **Active queue** в инспекторе свойств любого действия агента, чтобы собрать актуальные задачи на первых клавишах. Настройка действует сразу для всех действий от **Agent 1** до **Agent 6** на этом компьютере и по умолчанию выключена. Очередь фильтрует только текущий набор из максимум шести задач, уже выбранных и маршрутизированных из Codex: сначала идут задачи, требующие внимания, и ошибки, затем завершённые и непрочитанные задачи в порядке поступления при наличии времени активности, а после них — работающие задачи от самых свежих к более старым. Задачи в состоянии простоя или `off` скрываются, остальные позиции сдвигаются без пропусков.

Чтобы получить наиболее понятный набор кандидатов, выберите в Codex режим **Most recent chats**. Встроенный режим **Priority chats** может вернуть шесть задач в состоянии простоя, оставив работающую задачу за пределами этой шестёрки. **Active queue** не умеет обнаруживать такую пропущенную задачу, поэтому в этом случае все исправные клавиши агентов будут чёрными. Исправная чёрная позиция ничему не назначена и не реагирует на нажатие; сообщения о подключении, ухудшении связи и недоступности узла остаются видимыми.

Если в профиле N клавиш агентов, расположите логические действия **Agent 1** … **Agent N** подряд и по порядку. Пока очередь включена, открыть простаивающие разговоры этими клавишами нельзя. Режимы **pinned** и **custom** по-прежнему определяют набор кандидатов, но их фиксированные физические позиции уплотняются и могут перемещаться. Выключение **Active queue** полностью восстанавливает прежнюю раскладку источника агентов для одного или нескольких узлов.
