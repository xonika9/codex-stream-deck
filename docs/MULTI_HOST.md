...
Place logical **Agent 1** through **Agent N** contiguously on a profile with N Agent buttons. While the queue is enabled, attention/errors come first, then completion/unread tasks retain their existing FIFO order when activity times are available, then working tasks are ordered by their latest trustworthy user-start event. Opening or selecting a mirror, background reasoning, tool or assistant output, title changes, renderer activity, and refreshes do not reorder working keys. A newer user start from the exact rollout owner does. Idle chats are unavailable from the Agent buttons. Pinned and unpinned tasks participate in the full catalog; Individual/custom keeps its configured candidate set instead of expanding to that catalog.

### Ownership and SSH mirrors

Codex's built-in remote-SSH feature can mirror a task into the other renderer. Codex Deck does not confuse that CLI connection with the owning desktop. In multi-host mode it compares exact local rollout **filenames** on both hosts and checks only bounded 512 KiB JSONL tails for structural lifecycle tags and numeric metadata. Working order uses only the type, timestamp, and byte offset of the latest `event_msg` whose `payload.type` is `user_message`; the offset is a content-free event revision. A renderer `turn_context` alone never marks a completed task as working. Message text, responses, project names, and other rollout content are neither parsed nor sent through the relay.

The relay catalog contains only recent task UUIDs, modification times, derived `working`, `complete`, or `idle` state, an optional context-window percentage, and the optional atomic numeric pair `workStartedAt` / `workStartRevision`. This additive pair remains in relay protocol version 1: old senders omit it, receivers validate it and normalize its timestamp to the receiver clock. For the same cloud task visible on both hosts, live status, context usage, and selection are merged while commands route to the rollout owner—even when Codex temporarily omits the task from that owner's six native Micro slots. Only that exact owner supplies the start pair; mirror activity cannot reorder the task. Ownership is host-generic and contains no hard-coded task IDs or project names.
...
Working order has a narrower boundary. Starts found in the bounded tail, or retained in memory for up to 24 hours after observation, rank ahead of unknown starts. An older sender, a cold start, or a start outside the tail produces a stable queue-local fallback; first seeing a task already working is only a seed, not a claimed start time. A later idle/completion-to-working transition can raise an unknown task within the unknown tier. Disabling and re-enabling Active queue or restarting the plugin begins a new queue epoch.
